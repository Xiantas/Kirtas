...
fn list_files(path: &str) -> Vec<TodoItem> {
    let metadata = std::fs::metadata(path).expect("Given path is invalid");

    if metadata.is_dir() {
        std::fs::read_dir(path)
            .expect("Shouldnt crash, but whatever !")
            .map(|entry| {
                let entry = entry.unwrap();
                TodoItem {
                    todo: entry.file_name().to_str().unwrap().to_string(),
                    info: if let Ok(ft) = entry.file_type() {
                        if ft.is_dir() {
                            "Dir".to_string()
                        } else if ft.is_file() {
                            "File".to_string()
        ...
        vec![TodoItem {
            todo: path.file_name().unwrap().to_str().unwrap().to_string(),
            info: "Dir".to_string(),
            status: Status::Todo,
        }]
    } else {
        panic!("NTM un peu non à mettre des symlink ?");
    }
}
